...
// threadsafe variant
void
QDP$PC_$ABBR_eq_funcit($QDPPCTYPE *dest, void (*func)($NC$QLAPCTYPE($NCVAR(*dest)), int index), QDP_Subset subset)
{
#define N -1
#if ($C+0) == -1
  int nc = QDP_get_nc(dest);
...
void
QDP$PC_$ABBR_eq_funciat($QDPPCTYPE *dest, void (*func)($NC$QLAPCTYPE($NCVAR(*dest)), int index, void *args), void *args, QDP_Subset subset)
{
#define N -1
#if ($C+0) == -1
  int nc = QDP_get_nc(dest);
