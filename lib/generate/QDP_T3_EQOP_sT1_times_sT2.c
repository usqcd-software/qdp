  ...
  QDP_Subset subset)
{
  ...
  QDP_msg_tag *mtag1=NULL, *mtag2=NULL;
  TGET;
  ONE {
...
#define N -1
  ...
  if(subset->indexed==0) {
    fvpp($NCVAR QDP_offset_data(dest,toff), SRC1O(toff), SRC2O(toff), tlen );
  } else {
    fxpp($NCVAR QDP_offset_data(dest,0), SRC1O(0), SRC2O(0), subset->index, tlen );
  }
