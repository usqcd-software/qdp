#ifndef _QDP_COLLECT_INTERNAL_H
#define _QDP_COLLECT_INTERNAL_H
...
extern "C" {
  ...
  struct metaMessage {  /* meta message payload */
    ...
    int count;        /* number of elments to be sent */
  };
  ...
  struct QDP_Collect_struct {
    ...
    int *srcAddr;                    /*  A [srcSize] address of the src value in arena */
    int arenaSize;                   /*    number of elements in arena */
    int sndSize;                     /*    number of sends (may contain local node) */
    struct request *sndData;         /*  A [sndSize] send data (may contain local node) */
    ...
    QMP_msghandle_t *remoteMapH;     /* ^  [remoteCount] send map handles (only if remoteCount > 0) */
    struct request *remoteData;      /*  p [remoteCount] remote requests */
    int rcvCount;                    /*    number of nodes we receive data from */
...
#endif /* !defined(_QDP_COLLECT_INTERNAL_H) */
